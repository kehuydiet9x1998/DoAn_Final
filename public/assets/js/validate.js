  ...
  v.extend(validate, {
    ...
    processValidationResults: function (errors, options) {
      ...
      if (typeof v.formatters[format] === 'function') {
        ...
        throw new Error(v.format("không rõ định dạng %{format}", options));
      }
    ...
    convertErrorMessages: function (errors, options) {
      ...
      errors.forEach(function (errorInfo) {
        ...
        error = v.format(error, {
          value: v.stringifyValue(errorInfo.value)
        });
  ...
  validate.validators = {
    presence: function (value, options) {
      ...
      if (options.allowEmpty ? !v.isDefined(value) : v.isEmpty(value)) {
        return options.message || this.message || "không được để trống";
      }
    },
    length: function (value, options, attribute) {
      ...
      if (!v.isNumber(length)) {
        ...
        return options.message || this.notValid || "sai độ dài";
      }
      if (v.isNumber(is) && length !== is) {
        err = options.wrongLength || this.wrongLength || "sai độ dài ( %{count} ký tự)";
        errors.push(v.format(err, {
          count: is
        }));
      }
      if (v.isNumber(minimum) && length < minimum) {
        err = options.tooShort || this.tooShort || "quá ngắn (ít nhất %{count} ký tự)";
        errors.push(v.format(err, {
          count: minimum
        }));
      }
      if (v.isNumber(maximum) && length > maximum) {
        err = options.tooLong || this.tooLong || "quá dài (tối đa %{count} ký tự)";
        errors.push(v.format(err, {
          count: maximum
        }));
      }
    ...
    equality: function (value, options, attribute, attributes) {
      ...
      if (!comparator(value, otherValue, options, attribute, attributes)) {
        return v.format(message, {
          attribute: v.prettify(options.attribute)
        });
      }
    },
